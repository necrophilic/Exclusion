...
FILE *fd;
char payload[] =
"\x00\x01\x00\x00\x21\x12\xa4\x42\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\x00\r\n";
...
void *recievethread()
{
    ...
    while(1)
    {
        ...
        if(iph->protocol == 17)
        {
            ...
            unsigned char* payload = buffer + iphdrlen + 22;
            if(ntohs(udph->source) == 3478)
            {
                int body_length = data_size - iphdrlen - 22;
                if (body_length > 40)
                {
                found_srvs++;
